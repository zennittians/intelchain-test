...
intelchain_dir="$(go env GOPATH)/src/github.com/zennittians/intelchain"
...
function kill_localnet() {
  pushd "$(pwd)"
  cd "$intelchain_dir" && bash ./test/kill_node.sh
  popd
}

function setup() {
  if [ ! -d "$intelchain_dir" ]; then
    echo "Test setup FAILED: Missing intelchain directory at $intelchain_dir"
    exit 1
  fi
...
function build_and_start_localnet() {
  local localnet_log="$intelchain_dir/localnet_deploy.log"
  rm -rf "$intelchain_dir/tmp_log*"
  rm -rf "$intelchain_dir/.dht*"
  rm -f "$localnet_log"
  rm -f "$intelchain_dir/*.rlp"
  pushd "$(pwd)"
  cd "$intelchain_dir"
  if [ "$BUILD" == "true" ]; then
...
function wait_for_localnet_boot() {
  ...
  until $valid; do
    result=$(curl --silent --location --request POST "localhost:9500" \
      --header "Content-Type: application/json" \
      --data '{"jsonrpc":"2.0","method":"itc_blockNumber","params":[],"id":1}' | jq -r '.result')
    if ((result>0)); then
      valid=true
    else 
...
function wait_for_epoch() {
  ...
  until ((cur_epoch >= "$1")); do
    ...
    --header "Content-Type: application/json" \
    --data '{"jsonrpc":"2.0","method":"itcv2_latestHeader","params":[],"id":1}' | jq .result.epoch)
    if ((i > timeout)); then
      echo "TIMEOUT REACHED"
      exit 1
    fi
