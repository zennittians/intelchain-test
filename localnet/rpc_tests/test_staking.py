...
def test_get_all_validator_addresses(s0_validator, s1_validator):
    ...
    # Check v1
    raw_response = base_request("itc_getAllValidatorAddresses", params=[], endpoint=endpoints[beacon_shard_id])
    ...
    # Check v2
    raw_response = base_request("itcv2_getAllValidatorAddresses", params=[], endpoint=endpoints[beacon_shard_id])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    assert s0_validator["validator-addr"] in response, f"Expected validator {s0_validator['validator-addr']} " \
                                                       f"in validator list {response}"
...
def test_get_transaction_receipt_v1(s0_validator):
    ...
    raw_response = base_request("itc_getTransactionReceipt",
...
def test_get_transaction_receipt_v2(s0_validator):
    ...
    raw_response = base_request("itcv2_getTransactionReceipt",
                                params=[s0_validator["hash"]],
                                endpoint=endpoints[beacon_shard_id])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
...
def test_get_staking_transactions_count(s0_validator):
    ...
    # Check v1, SENT
    raw_response = base_request("itc_getStakingTransactionsCount",
    ...
    # Check v1, SENT
    raw_response = base_request("itcv2_getStakingTransactionsCount",
                                params=[s0_validator["validator-addr"], "SENT"],
                                endpoint=endpoints[beacon_shard_id])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
...
def test_get_all_validator_information(s0_validator, s1_validator):
    ...
    # Check v1
    raw_response = base_request("itc_getAllValidatorInformation", params=[0], endpoint=endpoints[beacon_shard_id])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    ...
    # Check v2
    raw_response = base_request("itcv2_getAllValidatorInformation", params=[0], endpoint=endpoints[beacon_shard_id])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    found_s0, found_s1 = False, False
    for validator in response:
...
def test_get_validator_information(s0_validator):
    ...
    # Check v1
    raw_response = base_request("itc_getValidatorInformation", params=[s0_validator["validator-addr"]],
                                endpoint=endpoints[beacon_shard_id])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    ...
    # Check v2
    raw_response = base_request("itcv2_getValidatorInformation", params=[s0_validator["validator-addr"]],
                                endpoint=endpoints[beacon_shard_id])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    _assert_validator_info(s0_validator, response)
...
def test_get_validator_information_by_block_number(s0_validator):
    ...
    # Check v1
    raw_response = base_request("itc_getValidatorInformationByBlockNumber",
    ...
    # Check v2
    raw_response = base_request("itcv2_getValidatorInformationByBlockNumber",
...
def test_get_all_validator_information_by_block_number(s0_validator, s1_validator):
    ...
    # Check v1
    raw_response = base_request("itc_getAllValidatorInformationByBlockNumber", params=[0, hex(curr_block)],
                                endpoint=endpoints[beacon_shard_id])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    found_s0, found_s1 = False, False
    ...
    # Check v2
    raw_response = base_request("itcv2_getAllValidatorInformationByBlockNumber", params=[0, curr_block],
                                endpoint=endpoints[beacon_shard_id])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    found_s0, found_s1 = False, False
...
def test_get_elected_validator_addresses(s0_validator, s1_validator):
    ...
    # Check v1
    raw_response = base_request("itc_getElectedValidatorAddresses", params=[], endpoint=endpoints[beacon_shard_id])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    assert s0_validator["validator-addr"] in response, f"Expected validator {s0_validator['validator-addr']} " \
    ...
    # Check v2
    raw_response = base_request("itcv2_getElectedValidatorAddresses", params=[], endpoint=endpoints[beacon_shard_id])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    assert s0_validator["validator-addr"] in response, f"Expected validator {s0_validator['validator-addr']} " \
                                                       f"in elected validator list {response}"
...
def test_get_delegations_by_delegator(s1_validator):
    ...
    for delegator in validator_info["validator"]["delegations"]:
        ...
        raw_response = base_request("itc_getDelegationsByDelegator", params=[del_addr],
                                    endpoint=endpoints[beacon_shard_id])
        response = check_and_unpack_rpc_response(raw_response, expect_error=False)
        assert_valid_json_structure(reference_response, response)
        ...
        # Check v2
        raw_response = base_request("itcv2_getDelegationsByDelegator", params=[del_addr],
                                    endpoint=endpoints[beacon_shard_id])
        response = check_and_unpack_rpc_response(raw_response, expect_error=False)
        assert_valid_json_structure(reference_response, response)
        assert_no_null_in_list(response)
...
def test_get_delegations_by_delegator_by_block_number(s1_validator):
    ...
    for delegator in validator_info["validator"]["delegations"]:
        # Check v1
        del_addr = delegator["delegator-address"]
        raw_response = base_request("itc_getDelegationsByDelegatorByBlockNumber", params=[del_addr, hex(curr_block)],
                                    endpoint=endpoints[beacon_shard_id])
        response = check_and_unpack_rpc_response(raw_response, expect_error=False)
        assert_valid_json_structure(reference_response, response)
        assert_no_null_in_list(response)
        ...
        # Check v2
        raw_response = base_request("itcv2_getDelegationsByDelegatorByBlockNumber", params=[del_addr, curr_block],
                                    endpoint=endpoints[beacon_shard_id])
        response = check_and_unpack_rpc_response(raw_response, expect_error=False)
        assert_valid_json_structure(reference_response, response)
        assert_no_null_in_list(response)
...
def test_get_delegations_by_validator(s1_validator):
    ...
    # Check v1
    raw_response = base_request("itc_getDelegationsByValidator", params=[val_addr],
                                endpoint=endpoints[beacon_shard_id])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    assert_no_null_in_list(response)
    ...
    # Check v2
    raw_response = base_request("itcv2_getDelegationsByValidator", params=[val_addr],
                                endpoint=endpoints[beacon_shard_id])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    for del_delegator in response:
...
def test_get_current_utility_metrics(s0_validator):
    ...
    # Check v1
    raw_response = base_request("itc_getCurrentUtilityMetrics", params=[],
                                endpoint=endpoints[beacon_shard_id])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)

    # Check v2
    raw_response = base_request("itcv2_getCurrentUtilityMetrics", params=[],
                                endpoint=endpoints[beacon_shard_id])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
...
def test_get_median_raw_stake_snapshot(s0_validator):
    ...
    # Check v1
    raw_response = base_request("itc_getMedianRawStakeSnapshot", params=[], endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    ...
    # Check v2
    raw_response = base_request("itcv2_getMedianRawStakeSnapshot", params=[], endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    found_s0_winner, found_s0_candidate = False, False
    for val in response["epos-slot-winners"]:
...
def test_get_super_committees(s0_validator):
        ...
        "previous": {
            "quorum-deciders": {
                "shard-0": {
                    ...
                    "itc-voting-power": "0.679999999999999998",
    ...
    # Check v1
    raw_response = base_request("itc_getSuperCommittees", params=[], endpoint=endpoints[beacon_shard_id])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    ...
    # Check v2
    raw_response = base_request("itcv2_getSuperCommittees", params=[], endpoint=endpoints[beacon_shard_id])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    found_validator, found_key = False, False
    for member in response["current"]["quorum-deciders"]["shard-0"]["committee-members"]:
...
def test_get_staking_network_info(s0_validator):
    ...
    # Check v1
    raw_response = base_request("itc_getStakingNetworkInfo", params=[],
                                endpoint=endpoints[beacon_shard_id])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)

    # Check v2
    raw_response = base_request("itcv2_getStakingNetworkInfo", params=[],
                                endpoint=endpoints[beacon_shard_id])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
...
def test_get_validator_keys(s0_validator):
    ...
    # Check v1
    raw_response = base_request("itc_getValidatorKeys", params=[curr_epoch],
    ...
    # Check v1
    raw_response = base_request("itcv2_getValidatorKeys", params=[curr_epoch],
                                endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    assert s0_validator["pub-bls-key"] in response, f"Expected pub bls key {s0_validator['pub-bls-key']} in {response}"
...
def test_get_validators_v1(s0_validator, s1_validator):
    ...
    raw_response = base_request("itc_getValidators", params=[curr_epoch],
                                endpoint=endpoints[beacon_shard_id])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    found_s0, found_s1 = False, False
...
def test_get_validators_v2(s0_validator, s1_validator):
    ...
    raw_response = base_request("itcv2_getValidators", params=[curr_epoch],
                                endpoint=endpoints[beacon_shard_id])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    found_s0, found_s1 = False, False
...
def test_pending_staking_transactions_v1():
    ...
    while time.time() - start_time <= tx_timeout:
        raw_response = base_request("itc_pendingStakingTransactions", endpoint=endpoints[beacon_shard_id])
        response = check_and_unpack_rpc_response(raw_response, expect_error=False)
        assert_valid_json_structure(reference_response, response)
...
def test_pending_staking_transactions_v2():
    ...
    start_time = time.time()
    while time.time() - start_time <= tx_timeout:
        raw_response = base_request("itcv2_pendingStakingTransactions", endpoint=endpoints[beacon_shard_id])
        response = check_and_unpack_rpc_response(raw_response, expect_error=False)
        assert_valid_json_structure(reference_response, response)
        for pending_tx in response:
            if pending_tx["hash"] == stx["hash"]:
...
def test_get_blocks_v1(s0_validator):
    ...
    raw_response = base_request("itc_getBlocks",
...
def test_get_blocks_v2(s0_validator):
    ...
    raw_response = base_request("itcv2_getBlocks",
                                params=[start_blk, end_blk, {
                                    "fullTx": True,
                                    "inclStaking": True
                                }],
...
def test_get_staking_transaction_history_v1(s0_validator):
    ...
    try:
        raw_response = base_request("itc_getStakingTransactionsHistory",
                                    params=[{
...
def test_get_staking_transaction_history_v2(s0_validator):
    ...
    raw_response = base_request("itcv2_getStakingTransactionsHistory",
                                params=[{
                                    "address": s0_validator["validator-addr"],
                                    "pageIndex": 0,
                                    "pageSize": 1000,
