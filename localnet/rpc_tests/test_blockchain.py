...
def test_net_peer_count_v1():
    """
    Note that this is NOT a `itc` RPC, however, there are 2 versions of it.
    """
...
def test_get_node_metadata():
    ...
    reference_response = {
        ...
        "version": "intelchain (C) 2020. intelchain, version v6110-v2.1.9-34-g24ec31c1 (danielvdm@ 2020-07-11T05:03:50-0700)",
        "network": "localnet",
        "chain-config": {
            "chain-id": 2,
            "cross-tx-epoch": 0,
    ...
    # Check v1
    raw_response = base_request("itc_getNodeMetadata", endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    ...
    # Check v2
    raw_response = base_request("itcv2_getNodeMetadata", endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    assert response["shard-id"] == 0
    assert response["network"] == "localnet"
...
def test_get_sharding_structure():
    ...
    # Check v1
    raw_response = base_request("itc_getShardingStructure", endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    ...
    # Check v2
    raw_response = base_request("itcv2_getShardingStructure", endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert isinstance(response, list), f"Expected response to be of type list, not {type(response)}"
    for d in response:
        assert isinstance(d, dict), f"Expected type dict in response elements, not {type(d)}"
...
def test_get_latest_header():
    ...
    # Check v1
    raw_response = base_request("itc_latestHeader", endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    assert response["shardID"] == 0

    # Check v2
    raw_response = base_request("itcv2_latestHeader", endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    assert response["shardID"] == 0
...
def test_get_latest_chain_headers():
    ...
    raw_response = base_request(
        "itc_getLatestChainHeaders", endpoint=endpoints[1])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    ...
    # Check v2
    raw_response = base_request("itcv2_getLatestChainHeaders", endpoint=endpoints[1])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    assert response["beacon-chain-header"]["shardID"] == 0
    assert response["shard-chain-header"]["shardID"] == 1
...
def test_get_leader_address():
    ...
    # Check v1
    raw_response = base_request("itc_getLeader", endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    ...
    # Check v2
    raw_response = base_request("itcv2_getLeader", endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    if response.startswith("one1"):
        assert account.is_valid_address(response), f"Leader address is not a valid ONE address"
    else:
...
def test_get_block_signer_keys():
    ...
    # Check v1
    raw_response = base_request("itc_getBlockSignerKeys", params=["0x2"], endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)

    # Check v2
    raw_response = base_request("itcv2_getBlockSignerKeys", params=[2], endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
...
def test_get_header_by_number():
    ...
    # Check v1
    raw_response = base_request("itc_getHeaderByNumber", params=["0x0"], endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    assert response["shardID"] == 0

    # Check v2
    raw_response = base_request("itcv2_getHeaderByNumber", params=[0], endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    assert response["shardID"] == 0
...
def test_get_block_signers():
    ...
    # Check v1
    raw_response = base_request("itc_getBlockSigners", params=["0x2"], endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    assert len(response) > 0, "expect at least 1 signer"

    # Check v2
    raw_response = base_request("itcv2_getBlockSigners", params=[2], endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    assert len(response) > 0, "expect at least 1 signer"
...
def test_get_block_number_v1():
    raw_response = base_request("itc_blockNumber", endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert isinstance(response, str) and response.startswith("0x")  # Must be a hex string


def test_get_block_number_v2():
    raw_response = base_request("itcv2_blockNumber", endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert isinstance(response, int)  # Must be an integer in base 10


def test_get_epoch_v1():
    raw_response = base_request("itc_getEpoch", endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert isinstance(response, str) and response.startswith("0x")  # Must be a hex string


def test_get_epoch_v2():
    raw_response = base_request("itcv2_getEpoch", endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert isinstance(response, int)  # Must be an integer


def test_get_gas_price_v1():
    raw_response = base_request("itc_gasPrice", endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert isinstance(response, str) and response.startswith("0x")  # Must be a hex string


def test_get_gas_price_v2():
    raw_response = base_request("itcv2_gasPrice", endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert isinstance(response, int)  # Must be an integer in base 10


def test_get_protocol_version_v1():
    raw_response = base_request("itc_protocolVersion", endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert isinstance(response, str) and response.startswith("0x")  # Must be a hex string


def test_get_protocol_version_v2():
    raw_response = base_request("itcv2_protocolVersion", endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert isinstance(response, int)  # Must be an integer in base 10
...
def test_get_block_transaction_count_by_number_v1():
    ...
    raw_response = base_request("itc_getBlockTransactionCountByNumber", params=[init_tx["blockNumber"]],
...
def test_get_block_transaction_count_by_number_v2():
    ...
    raw_response = base_request("itcv2_getBlockTransactionCountByNumber", params=[int(init_tx["blockNumber"], 16)],
                                endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
...
def test_get_block_transaction_count_by_hash_v1():
    ...
    raw_response = base_request("itc_getBlockTransactionCountByHash", params=[init_tx["blockHash"]],
                                endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert isinstance(response, str) and response.startswith("0x")  # Must be a hex string
    assert int(response, 16) > 0, "Expected transaction count > 0 due to initial transactions"
...
def test_get_block_transaction_count_by_hash_v2():
    ...
    raw_response = base_request("itcv2_getBlockTransactionCountByHash", params=[init_tx["blockHash"]],
                                endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert isinstance(response, int)
    assert response > 0, "Expected transaction count > 0 due to initial transactions"
...
def test_get_block_by_number_v1():
    ...
    raw_response = base_request("itc_getBlockByNumber", params=["0x0", True], endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
...
def test_get_block_by_number_v2():
    ...
    raw_response = base_request("itcv2_getBlockByNumber", params=[0, {"InclStaking": True}], endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
...
def test_get_block_by_hash_v1():
    ...
    init_tx_record = initial_funding[0]
    init_tx = transaction.get_transaction_by_hash(init_tx_record["hash"], endpoints[init_tx_record["from-shard"]])
    raw_response = base_request("itc_getBlockByHash",
...
def test_get_block_by_hash_v2():
    ...
    init_tx_record = initial_funding[0]
    init_tx = transaction.get_transaction_by_hash(init_tx_record["hash"], endpoints[init_tx_record["from-shard"]])
    raw_response = base_request("itcv2_getBlockByHash",
