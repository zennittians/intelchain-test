...
def test_get_pool_stats():
    ...
    raw_response = base_request("itc_getPoolStats", params=[], endpoint=endpoints[0])
    ...
    raw_response = base_request("itcv2_getPoolStats", params=[], endpoint=endpoints[0])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
...
def test_get_current_transaction_error_sink():
    ...
    response = base_request('itc_sendRawTransaction', params=[error_tx["signed-raw-tx"]],
    ...
    # Check v1
    raw_response = base_request("itc_getCurrentTransactionErrorSink", params=[],
                                endpoint=endpoints[error_tx["from-shard"]])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
    ...
    # Check v2
    raw_response = base_request("itcv2_getCurrentTransactionErrorSink", params=[],
                                endpoint=endpoints[error_tx["from-shard"]])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
...
def test_resend_cx(cross_shard_txs):
    ...
    for tx in cross_shard_txs:
        raw_response = base_request("itc_resendCx", params=[tx["hash"]],
                                    endpoint=endpoints[tx["shardID"]])
        response = check_and_unpack_rpc_response(raw_response, expect_error=False)
        assert_valid_json_structure(reference_response, response)

        raw_response = base_request("itcv2_resendCx", params=[tx["hash"]],
                                    endpoint=endpoints[tx["shardID"]])
        response = check_and_unpack_rpc_response(raw_response, expect_error=False)
        assert_valid_json_structure(reference_response, response)
...
def test_get_pending_cx_receipts():
    ...
    while time.time() - start_time <= tx_timeout * 2:  # Cross shards are generally slower...
        if not v1_success:
            raw_response = base_request("itc_getPendingCXReceipts", endpoint=endpoints[cx["to-shard"]])
            response = check_and_unpack_rpc_response(raw_response, expect_error=False)
            assert_valid_json_structure(reference_response, response)
        ...
        if not v2_success:
            raw_response = base_request("itcv2_getPendingCXReceipts", endpoint=endpoints[cx["to-shard"]])
            response = check_and_unpack_rpc_response(raw_response, expect_error=False)
            assert_valid_json_structure(reference_response, response)
            for cx_receipt in response:
                for r in cx_receipt["receipts"]:
...
def test_get_cx_receipt_by_hash_v1(cross_shard_txs):
    ...
    raw_response = base_request("itc_getCXReceiptByHash", params=[cross_shard_txs[0]["hash"]],
...
def test_get_cx_receipt_by_hash_v2(cross_shard_txs):
    ...
    raw_response = base_request("itcv2_getCXReceiptByHash", params=[cross_shard_txs[0]["hash"]],
                                endpoint=endpoints[cross_shard_txs[0]["toShardID"]])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
...
def test_send_raw_transaction_v1():
    ...
    response = base_request('itc_sendRawTransaction', params=[tx["signed-raw-tx"]],
    ...
    while time.time() - start_time <= tx_timeout:
        if get_transaction(tx["hash"], tx["from-shard"]) is not None:
            raw_response = base_request('itc_sendRawTransaction', params=[tx["signed-raw-tx"]],
                                        endpoint=endpoints[tx["from-shard"]])
...
def test_send_raw_transaction_v2():
    ...
    # Submit new transaction...
    response = base_request('itcv2_sendRawTransaction', params=[tx["signed-raw-tx"]],
                            endpoint=endpoints[tx["from-shard"]])
    tx_hash = check_and_unpack_rpc_response(response, expect_error=False)
    assert tx_hash == tx["hash"], f"Expect submitted transaction to get tx hash of {tx['hash']}, got {tx_hash}"
    ...
    while time.time() - start_time <= tx_timeout:
        if get_transaction(tx["hash"], tx["from-shard"]) is not None:
            raw_response = base_request('itcv2_sendRawTransaction', params=[tx["signed-raw-tx"]],
                                        endpoint=endpoints[tx["from-shard"]])
            response = check_and_unpack_rpc_response(raw_response, expect_error=True)
            assert_valid_json_structure(reference_response, response)
            assert reference_response["code"] == response["code"], f"Expected error code {reference_response['code']}, " \
...
def test_get_transaction_by_hash_v1():
    ...
    raw_response = base_request("itc_getTransactionByHash",
...
def test_get_transaction_by_hash_v2():
    ...
    raw_response = base_request("itcv2_getTransactionByHash",
                                params=[init_tx_record["hash"]],
                                endpoint=endpoints[init_tx_record["from-shard"]])
    response = check_and_unpack_rpc_response(raw_response, expect_error=False)
    assert_valid_json_structure(reference_response, response)
...
def test_pending_transactions_v1():
    ...
    start_time = time.time()
    while time.time() - start_time <= tx_timeout:
        raw_response = base_request("itc_pendingTransactions", endpoint=endpoints[tx["from-shard"]])
        response = check_and_unpack_rpc_response(raw_response, expect_error=False)
        assert_valid_json_structure(reference_response, response)
...
def test_pending_transactions_v2():
    ...
    start_time = time.time()
    while time.time() - start_time <= tx_timeout:
        raw_response = base_request("itcv2_pendingTransactions", endpoint=endpoints[tx["from-shard"]])
        response = check_and_unpack_rpc_response(raw_response, expect_error=False)
        assert_valid_json_structure(reference_response, response)
        for pending_tx in response:
            if pending_tx["hash"] == tx["hash"]:
...
def test_get_transaction_by_block_hash_and_index_v1():
    ...
    raw_response = base_request("itc_getTransactionByBlockHashAndIndex",
...
def test_get_transaction_by_block_hash_and_index_v2():
    ...
    raw_response = base_request("itcv2_getTransactionByBlockHashAndIndex",
...
def test_get_transaction_by_block_number_and_index_v1():
    ...
    raw_response = base_request("itc_getTransactionByBlockNumberAndIndex",
...
def test_get_transaction_by_block_number_and_index_v2():
    ...
    raw_response = base_request("itcv2_getTransactionByBlockNumberAndIndex",
