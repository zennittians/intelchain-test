...
def send_transaction(tx_data, confirm_submission=False):
    ...
    # Send tx
    response = base_request('itc_sendRawTransaction', params=[tx_data["signed-raw-tx"]],
...
def send_staking_transaction(tx_data, confirm_submission=False):
    ...
    # Send tx
    response = base_request('itc_sendRawStakingTransaction', params=[tx_data["signed-raw-tx"]],
...
def get_transaction(tx_hash, shard):
    ...
    raw_response = base_request('itc_getTransactionByHash', params=[tx_hash], endpoint=endpoints[shard])
...
def get_staking_transaction(tx_hash):
    ...
    """
    assert isinstance(tx_hash, str), f"Sanity check: expect tx hash to be of type str not {type(tx_hash)}"
    raw_response = base_request('itc_getStakingTransactionByHash', params=[tx_hash], endpoint=endpoints[0])
    return check_and_unpack_rpc_response(raw_response, expect_error=False)
